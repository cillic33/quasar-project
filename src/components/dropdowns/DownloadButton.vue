<template>
  <div class="wrapper" id="downloadBtnsBlock">
    ...
    <q-list v-if="downloadPopup" class="popup">
      <q-item clickable v-close-popup @click="onExelClick">
        Excel
      </q-item>
      <q-item clickable v-close-popup @click="onPdfClick">
        PDF
      </q-item>
    </q-list>
  </div>
...
export default {
  setup () {
    ...
    return {
      downloadPopup,
      onExelClick () {
        alert('Clicked on an Exel Item')
      },
      onPdfClick () {
        alert('Clicked on an PDF Item')
      },
    }
  },
  components: {
